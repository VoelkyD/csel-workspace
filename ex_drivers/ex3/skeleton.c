...
static ssize_t skeleton_read(struct file *f, char __user *buf, size_t count, loff_t *off)
{
    ...
    ssize_t remaining = dev->len - *off;
    if (remaining <= 0) return 0;
    if (count > remaining) count = remaining;

    if (copy_to_user(buf, dev->buffer + *off, count))
        return -EFAULT;

    *off += count;
    pr_info("skeleton: read %ld bytes (offset now %lld)\n", count, *off);
    return count;
}
...
static ssize_t skeleton_write(struct file *f, const char __user *buf, size_t count, loff_t *off)
{
    ...
    // Forcément recommencer à 0 (pour écraser entièrement l'ancien message)
    *off = 0;
    ...
    *off += count;
    ...
    pr_info("skeleton: wrote %ld bytes (offset now %lld, len=%ld)\n", count, *off, dev->len);
    return count;
}
